...
import { companyLogoForName, downloadBrandedPdf } from "@/lib/branded-pdf";
import {
  type Conversation,
  type BusinessMemory,
  loadConversations,
...
const QUICK_ACTIONS: QuickAction[] = [
  ...
  { icon: "📑", label: "Create Quotation", prompt: "Create a professional quotation for a South African client. Ask me for the issuing company, client, line items, prices, quantities, payment terms, validity and VAT status before generating it." },
...
function AssistantMessage({ content }: { content: string }) {
  const cleaned = cleanGeneratedDocument(content);
  return (
...
function isDownloadableDocument(content: string): boolean {
  return /^(QUOTATION|INVOICE|TAX INVOICE|CONTRACT|AGREEMENT|PROPOSAL|METHOD STATEMENT|RISK ASSESSMENT)\b/i
    .test(cleanGeneratedDocument(content));
}

function downloadText(name: string, content: string) {
  const blob = new Blob([cleanGeneratedDocument(content)], { type: "text/plain;charset=utf-8" });
              ...
              {m.role === "user" ? (
                ...
                <div className="w-full max-w-full rounded-2xl rounded-bl-sm border border-border/50 bg-background/50 px-4 py-3">
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5 text-[11px]">
                    <button onClick={() => navigator.clipboard.writeText(cleanGeneratedDocument(m.content))} className="rounded-md border border-border px-2 py-1 hover:border-gold/60">Copy</button>
                    {isDownloadableDocument(m.content) && (
                      <>
                        <button
                          onClick={() => downloadBrandedPdf({
                            title: cleanGeneratedDocument(m.content).split("\n")[0] || activeConv?.title || "NexDocs Document",
                            content: m.content,
                            brand: {
                              companyName: memory.companyName,
                              companyLogo: companyLogoForName(memory.companyName),
                          ...
                          className="rounded-md border border-border px-2 py-1 hover:border-gold/60"
                        >
                          Download PDF
                        </button>
